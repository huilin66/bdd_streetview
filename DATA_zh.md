...
python pan_tools/visualize.py --html         # 仅 HTML
python pan_tools/visualize.py --shp          # 仅 Shapefile
```

HTML: Leaflet + Canvas 叠加层，像素去重渲染，街道/卫星底图切换，自动启动本地 HTTP 服务器。

Shapefile: ESRI 点要素 (WGS84)，属性含 `id`、`lng`、`lat`、`elev_m`。
